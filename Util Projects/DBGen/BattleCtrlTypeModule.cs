...
namespace DBGen
{
    internal static class BattleCtrlTypeModule
    {
        public static void Procedure()
        {
            ...
            Dictionary<string, ushort> m_cfcDict = new Dictionary<string, ushort>();
            
            foreach(var cfc in cfcList)
            {
                m_cfcDict.Add(cfc.Name, (ushort)cfc.Last);
            }
            ...
            foreach(string cfeFile in list)
            {
                ...
                ArmpEntry entry = ctrlTypeBin.GetMainTable().AddEntry(Path.GetFileNameWithoutExtension(cfeFile));
